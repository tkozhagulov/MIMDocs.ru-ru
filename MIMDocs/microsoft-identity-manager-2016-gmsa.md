...
3. После этого в службах синхронизации создайте резервную копию ключа шифрования, которая потребуется при установке с изменением режима.
    ...
    -   Итак, вы правильно ввели все сведения об учетной записи. Теперь вы можете изменить место назначения (расположение файла экспорта) для резервной копии ключа шифрования.
        ...
        -   По умолчанию файл будет экспортирован по адресу  **C:\\Windows\\system32**\\miiskeys-1.bin.
...
3.  **Примечание.**  Существует известная проблема, приводящая к зависанию служб, использующих управляемую учетную запись, после перезагрузки сервера. Она возникает из-за того, что служба распространения ключей Майкрософт не запускается после перезагрузки Windows. При этом запустить службу или перезапустить Windows невозможно. Проблему можно воспроизвести по крайней мере на Windows Server 2012 R2. Чтобы решить эту проблему, выполните команду 
