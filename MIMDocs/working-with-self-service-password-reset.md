...
-   Теперь на портале самостоятельного сброса пароля и экране входа в Windows можно разблокировать свою учетную запись, не меняя пароль, или обратиться к администраторам технической поддержки. Учетные записи пользователей блокируются по многим веским причинам, например если пользователь ввел старый пароль, на многоязычном компьютере выбран неправильный язык или вход выполнялся на общей рабочей станции, которая уже открыта для другой учетной записи.

-   Добавлен новый шлюз проверки подлинности, "Телефонный шлюз". Этот шлюз позволяет выполнять проверку подлинности пользователей по телефону.

-   Добавлена поддержка службы многофакторной идентификации Microsoft Azure (MFA). Эту службу можно использовать как для существующего шлюза одноразового SMS-пароля, так и для нового телефонного шлюза.
...
В следующем разделе вы настроите поставщика многофакторной проверки подлинности Azure в Microsoft Azure Active Directory. Теперь создайте файл с данными для проверки подлинности, которые требуются службе MFA для подключения к серверу Azure MFA.  Для продолжения работы потребуется подписка Azure.
...
1.  Создайте [поставщик MFA](https://docs.microsoft.com/azure/multi-factor-authentication/multi-factor-authentication-get-started-auth-provider).
...
8.  В разделе **Предупреждение о мошенничестве** снимите флажок ** "Блокировать пользователя при получении сообщения о мошенничестве". Это позволит избежать блокирования всей службы.
...
10. В элементе `<DefaultCountryCode>` введите код страны по умолчанию. Если при регистрации номера телефона для пользователя код страны не будет указан, пользователю присваивается этот код. Если у пользователя есть международный код страны, его необходимо включить в номер телефона при регистрации.
...
1.  Запустите Internet Explorer, перейдите на портал MIM, войдите как администратор службы MIM и щелкните **Рабочие процессы** на панели навигации слева.
...
#### <a name="windows-desktop-login-integrated-password-reset"></a>Интегрированный процесс сброса пароля с помощью рабочего стола Windows
