...
Как правило, разработка стратегии резервного копирования и восстановления выполняется совместно с администратором базы данных. Вот некоторые рекомендации:
- Резервное копирование баз данных должно выполняться в соответствии с корпоративной политикой резервного копирования. 
- Если добавочное резервное копирование журналов не запланировано, для базы данных следует задать простой режим восстановления. 
- Перед реализацией стратегии резервного копирования необходимо узнать обо всех моделях восстановления, а также о предъявляемых ими требованиях к пространству на дисках. Модель полного восстановления требует частого резервного копирования журналов во избежание чрезмерного использования дискового пространства. 

Дополнительные сведения см. в разделе, содержащем [общие сведения о моделях восстановления](http://go.microsoft.com/fwlink/?LinkID=185370), и в [руководстве по резервному копированию и восстановлению FIM 2010](http://go.microsoft.com/fwlink/?LinkID=165864).

## <a name="create-a-backup-administrator-account-for-the-fim-service-after-installation"></a>Создание учетной записи администратора резервного копирования для службы FIM после установки

Участники группы администраторов службы FIM имеют уникальные разрешения, необходимые для работы с развертыванием MIM. Если вы не можете войти как участник группы администраторов, единственное решение — выполнить откат к предыдущей резервной копии системы. Чтобы избежать возникновения такой ситуации, рекомендуем добавить других пользователей в группу администраторов FIM в рамках настройки после установки.
...
Рекомендуем отключить индексирование Microsoft Office SharePoint®. Документы, которые должны быть проиндексированы, отсутствуют. Индексирование приводит к ошибкам в записях журнала и может вызывать проблемы с производительностью в MIM. Отключить индексирование SharePoint можно так.
...
В этом разделе приведены инструкции по повышению производительности загрузки начальных данных из внешней системы в MIM. Важно понимать, что некоторые из этих действий выполняются только в ходе начального заполнения системы данными. Их следует сбросить после завершения загрузки. Это разовая операция, которая не служит для непрерывной синхронизации.

> [!NOTE]
> Для получения дополнительных сведений о синхронизации пользователей между MIM и доменными службами Active Directory (AD DS) см. соответствующий [раздел](http://go.microsoft.com/fwlink/?LinkID=188277) документации по FIM.
> 
> [!IMPORTANT]
...
Начальная загрузка данных может быть продолжительной. Если изначально требуется загрузить большой объем данных, заполнение базы данных можно ускорить, временно отключив полнотекстовый поиск и включив его снова после завершения экспорта в агенте управления MIM 2016.
...
На этом этапе необходимо выполнить процедуры, описанные в руководстве по синхронизации пользователей между доменными службами Active Directory и FIM, для настройки и синхронизации в системе пользователей из Active Directory. Если необходимо синхронизировать сведения о группах, см. руководство по [синхронизации групп из доменных служб Active Directory в FIM](https://technet.microsoft.com/library/ff686936(v=ws.10).aspx).

#### <a name="synchronization-and-export-sequences"></a>Последовательность синхронизации и экспорта

Чтобы оптимизировать производительность, после прохода синхронизации, который привел к образованию большого количества ожидающих выполнения операций экспорта в пространстве соединителя, выполните экспорт. Затем выполните операцию подтверждения импорта в агенте управления, связанном с соответствующим пространством соединителя. Например, если в рамках начальной загрузки данных необходимо выполнить профили запуска синхронизации в нескольких агентах управления, после каждого прохода синхронизации следует произвести экспорт, а затем разностный импорт.
...
8.  Перейдите к https://servername/certsrv. Замените servername на имя сервера, выдающего сертификаты.
...
29. Щелкните ссылку http://servername.

30. Измените http://servername на https://servername, а затем нажмите кнопку "ОК".
...
-   отключите индексирование SharePoint на сайте портала MIM. Дополнительные сведения см. в разделе "Отключение индексирования SharePoint" этого документа.

## <a name="feature-specific-best-practices"></a>Рекомендации для отдельных компонентов 
...
- Правило политики управления "Запрос"
  ...
  - Применяется при выполнении операций создания, чтения, обновления или удаления в отношении целевого ресурса в MIM.
...
При применении конфигурации используйте принцип наименьших привилегий. Правила политики управления управляют доступом к развертыванию MIM. Включайте только те функции, которые нужны большинству пользователей. Например, если не все пользователи применяют MIM для управления группами, соответствующие правила политики управления группами следует отключить. По умолчанию в MIM большинство разрешений, не связанных с работой администраторов, отключены.
...
В MIM разрешения определяются как положительное утверждение. Так как MIM не поддерживает запрет разрешений, предоставление неограниченного доступа к ресурсу усложняет настройку исключений из разрешений. Рекомендуется предоставлять только необходимые разрешения.
...
Чтобы удалить назначение, но оставить текущие элементы (например, чтобы прекратить использование MIM для управления назначением), выполните указанные ниже действия.
...
При развертывании политики сброса паролей MIM на рабочей станции, предназначенной для работы в режиме терминала, рекомендуется включить параметр локальной политики безопасности "Завершение работы: очистка файла подкачки виртуальной памяти". Так вы запретите доступ к конфиденциальной информации из памяти процесса пользователям, не прошедшим аутентификацию.
...
Когда пользователь пытается зарегистрироваться для сброса пароля через веб-портал, MIM всегда инициирует регистрацию от имени пользователя, вошедшего в систему, независимо от того, кто вошел на веб-сайт. Пользователи должны всегда регистрироваться для сброса пароля на компьютерах, в которые они вошли.
...
Рабочий процесс авторизации не следует связывать с операцией сброса пароля. Для сброса пароля требуется синхронный ответ, а рабочие процессы авторизации, содержащие такие действия, как подтверждение, являются асинхронными.
...
В MIM можно отключить учет регистра в некоторых регулярных выражениях. Чтобы регистр игнорировался в группе, можно использовать ?!:. Например, для типа сотрудника используйте следующее выражение:
...
В MIM можно вводить строки с начальными и конечными пробелами, но система MIM игнорирует такие пробелы. При отправке строки с начальными и конечными пробелами они пропускаются обработчиком синхронизации и веб-службами.

#### <a name="empty-strings-do-not-equal-null"></a>Пустые строки не равны null.

В этой версии MIM пустые строки не равны null. Ввод пустой строки допускается в качестве значения. Отсутствие ввода рассматривается как значение null.
...
Следующие рабочие процессы входят в состав MIM и не должны удаляться:
...
Задачей MIM является обработка запросов, которая может инициироваться различными клиентами MIM, такими как служба синхронизации FIM и компоненты самообслуживания, в соответствии с настроенными бизнес-политиками. Каждый экземпляр службы FIM относится к логической группе, которая состоит из одного или нескольких экземпляров службы FIM. Такие группы также называются разделами службы FIM. Если развернут только один экземпляр службы FIM, который используется для обработки всех запросов, при обработке могут возникать задержки. Более того, при выполнении некоторых операций самообслуживания может истекать время ожидания. Разделы службы FIM могут помочь решить эту проблему.

См. дополнительные сведения о [разделах службы FIM](https://social.technet.microsoft.com/wiki/contents/articles/2363.understanding-fim-service-partitions.aspx).

## <a name="next-steps"></a>Дальнейшие шаги
- [Руководство по архивации и восстановлению FIM](http://go.microsoft.com/fwlink/?LinkID=165864)
- [Как синхронизировать пользователей Active Directory и FIM](http://go.microsoft.com/fwlink/?LinkID=188277) 
- [Обзор модели восстановления](http://go.microsoft.com/fwlink/?LinkID=185370)
